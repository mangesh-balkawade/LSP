...
{
    printk(KERN_INFO "Marvellous:Initializeing driver\n");

    majorNumber = register_chrdev(0, DEVICE_NAME, &fops);

    if (majorNumber < 0)
    {
        printk(KERN_ALERT "Marvellous:Failed to registered with major number\n");
        return majorNumber;
    }
    ...
    if (IS_ERR(charDevice))
    {
        ...
        unregister_chrdev(majorNumber, DEVICE_NAME);
        printk(KERN_ALERT "Marvellouclass_creates:Failed to create device\n");
...
static void __exit char_exit(void)
{
    ...
    class_destroy(charClass);
    unregister_chrdev(majorNumber, DEVICE_NAME);
    printk(KERN_INFO "Marvellous:Driver unload successfully \n");
}
...
static int dev_open(struct inode *inodep, struct file *filep)
{
    ...
    printk(KERN_INFO "Marvellous:Device open successfully %d time(s) \n", numberOpens);
...
static ssize_t dev_read(struct file *filep, char __user *buffer, size_t len, loff_t *offset)
{
    ...
    *offset += len;
    ...
    printk(KERN_INFO "Marvellous:Send %zu characters to user \n", len);
...
static ssize_t dev_write(struct file *filep, const char __user *buffer, size_t len, loff_t *offset)
{
    ...
    printk(KERN_INFO "Marvellous:Recieved %zu characters\n", len);
